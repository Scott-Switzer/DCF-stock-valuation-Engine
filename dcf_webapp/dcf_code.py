...
class DCFModel:
    ...
    def calculate_terminal_value(self, final_ufcf: float) -> float:
        """
        PHASE 4 (Helper): TERMINAL VALUE
        """
    ...
    def calculate_intrinsic_value(self) -> float:
        ...
        tv = self.calculate_terminal_value(ufcfs[-1])
        pv_tv = tv / ((1 + wacc) ** 5)
        ...
        # Step 7: Equity Value
        total_debt = self.data.total_debt[-1]
        cash = self.data.cash_and_equivalents[-1]
